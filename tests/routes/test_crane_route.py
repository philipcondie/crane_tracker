def test_create_crane_route(client):
    ...
    response = client.post("/cranes",json={
        "lat":10, "lng": 20, "project_name": "test_project", "status": "active",
    })
...
def test_get_cranes_route_filters_by_bounds(client):
    client.post("/cranes",json={
        "lat":5, "lng": 5, "project_name": "inside", "status": "active",
    })

    client.post("/cranes", json={
         "lat":50, "lng": 50, "project_name": "inside", "status": "active",
    })
